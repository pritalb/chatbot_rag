...
def create_and_populate_faiss_index(embeddings, index_path, docstore_path):
    ...
    documents = {
        str(i): Document(page_content=item["question"], metadata={"answer": item["answer"]}) for i, item in enumerate(embeddings)
    }
...
if __name__ == "__main__":
    ...
    index, docstore = create_and_populate_faiss_index(embeddings, index_path, metadata_path)
